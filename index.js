...
app.get("/students/topper",(req,res)=>{
    if(students.length===0){
        return res.status(404).json({message:"no topper found"})
    }
    const topper=students.reduce((max,curr)=>{
        return curr.cgpa>max.cgpa?curr:max;
    },students[0]);
...
app.get("/students/average", (req, res) => {
  ...
  const average = Number((totalCgpa / students.length).toFixed(2)); 
...
app.get("/students/branch/:branchName", (req, res) => {
  const branchParams = req.params.branchName.toLowerCase();
  

  const filteredStudents = students.filter(
    student => student.branch.toLowerCase() === branchParams
  );


  if (filteredStudents.length === 0) {
    return res.status(404).json({ message: `No students found in the ${req.params.branchName} branch.` });
  }
